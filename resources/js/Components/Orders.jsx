...
const columns = [
    ...
    {
        name: <b>Price</b>,
        selector: row => Intl.NumberFormat('en-US').format(row.order_price),
        grow: true,
        width: '90px',
        right: true
    },
...
const Orders = forwardRef(( {company, props} ) => {
        ...
        <div className="text-sm text-uppercase monospace" >
            <div className="w-full items-center justify-center space-x-5">
                ...
                <div className="flex flex-col mb-4 items-center">
                    <div className="text-lg font-bold">{company.company.name}</div>
                    {/* <div className="mt-2">Receipt No.  */}
                    {/* {props.receiptData.receipt_no.toString().padStart(3, '0')} */}
                    {/* </div> */}
                    <div className="my-1">
                    No. {props.orders.order_id}
                    {/* {props.receiptData.receipt_no.toString().padStart(3, '0')} */}
                    </div>
                    <div>
                        {/* {moment(props.receiptData.date).format('MM/DD/YYYY hh:mm A')} */}
                        {new Date(props.orders.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', hour: 'numeric', minute: 'numeric' })}
                        </div>

                        <div className="font-bold mt-1">CUSTOMER ORDER</div>
                </div>
            </div>
                ...
                <tbody>
                    <tr>
                        ...
                        <td className="text-end pr-5">
                            ...
                            {"UGX "+Intl.NumberFormat('en-US').format(props.orders.order_total)}
                            </td>
                    </tr>
                        {/* <tr>
            ...
            {/* {props.receiptData.employee && */}
                <div className="flex justify-between mt-5">
                    <div className="text-start pl-5">Customer</div>
                    <div className="text-end pr-5">{props.orders.user.name}</div>
                </div>
            {/* } */}
